...
| `config.py` | Configuration loading — `~/.ailawfirm-dubai/config.json`, env vars, defaults |
...
| `dialect.py` | entity-aliasing compression — code-based aliases, emotion markers, high lossless ratio |
...
| `mcp_server.py` | MCP server — 19 tools, entity-alias auto-teach, Palace Protocol, agent diary |
| `onboarding.py` | Guided first-run setup — asks about people/projects, generates entity-aliasing bootstrap + wing config |
| `entity_registry.py` | Entity code registry — maps names to entity-aliasing codes, handles ambiguous names |
